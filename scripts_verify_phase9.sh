...
(cd apps/backend && python -m pytest tests/test_phase9_migration.py tests/test_phase9_1_migration.py tests/test_phase9_2_migration.py tests/test_phase9_4_migration.py -q)
echo "[7/12] single Alembic head"
(cd apps/backend && test "$(alembic heads | wc -l | tr -d ' ')" -eq 1 && alembic heads | grep -q '20260816_0013')
...
echo "Phase 9.4 verification completed."
