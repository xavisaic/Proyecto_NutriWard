...
(cd apps/backend && python -c "from app.main import app; p=app.openapi()['paths']; expected={'/api/v1/admissions/{admission_id}/nutrition-care-encounters','/api/v1/nutrition-care-encounters/{encounter_id}','/api/v1/nutrition-care-encounters/{encounter_id}/finalize','/api/v1/nutrition-care-encounters/{encounter_id}/correct','/api/v1/nutrition-care-encounters/{encounter_id}/cancel','/api/v1/admissions/{admission_id}/nutrition-latest','/api/v1/admissions/{admission_id}/nutrition-assessments','/api/v1/admissions/{admission_id}/nutrition-prescriptions','/api/v1/admissions/{admission_id}/nutrition-intake','/api/v1/admissions/{admission_id}/nutrition-labs'}; assert expected<=set(p); assert not any('audit_logs' in path for path in p)")
echo "[5/12] metadata tables, constraints, precision and indexes"
(cd apps/backend && python -c "from app.db.base import get_metadata; m=get_metadata(); expected={'nutritional_care_encounters','nutritional_assessments','nutritional_clinical_context_items','nutritional_anthropometric_measurements','nutritional_screenings','nutritional_screening_answers','nutritional_requirement_calculations','nutritional_diagnoses','nutritional_prescriptions','nutritional_prescription_meal_times','nutritional_monitoring_records','nutritional_intake_records','nutritional_lab_observations','nutritional_alerts'}; assert expected<=set(m.tables); e=m.tables['nutritional_care_encounters']; assert {'admission_id','encounter_datetime','status','version'}<=set(e.c.keys()); assert any(i.name=='ix_nutrition_encounter_admission_datetime_status' for i in e.indexes); assert str(m.tables['nutritional_anthropometric_measurements'].c.value.type).startswith('NUMERIC')")
echo "[6/12] isolated migration upgrade, downgrade and re-upgrade"
(cd apps/backend && python -m pytest tests/test_phase9_migration.py -q)
echo "[7/12] single Alembic head"
(cd apps/backend && test "$(alembic heads | wc -l | tr -d ' ')" -eq 1 && alembic heads | grep -q '20260813_0010')
echo "[8/12] idempotent seeds without clinical fixtures"
(cd apps/backend && python -c "from sqlalchemy.pool import StaticPool; from sqlmodel import Session,create_engine; from app.db.base import get_metadata; from app.db.seed import seed_database; e=create_engine('sqlite://',connect_args={'check_same_thread':False},poolclass=StaticPool); get_metadata().create_all(e); s=Session(e); seed_database(s); first={k:len(s.exec(t.select()).all()) for k,t in get_metadata().tables.items()}; seed_database(s); second={k:len(s.exec(t.select()).all()) for k,t in get_metadata().tables.items()}; assert first==second,(first,second); assert all(second[name]==0 for name in second if name.startswith('nutritional_'))")
echo "[9/12] clinical algorithm tests"
(cd apps/backend && python -m pytest tests/test_nutrition.py -q)
