...
try {
    ...
    python -c "from app.main import app; p=app.openapi()['paths']; expected={'/api/v1/admissions/{admission_id}/nutrition-care-encounters','/api/v1/nutrition-care-encounters/{encounter_id}','/api/v1/nutrition-care-encounters/{encounter_id}/finalize','/api/v1/nutrition-care-encounters/{encounter_id}/correct','/api/v1/nutrition-care-encounters/{encounter_id}/cancel','/api/v1/admissions/{admission_id}/nutrition-latest','/api/v1/admissions/{admission_id}/nutrition-assessments','/api/v1/admissions/{admission_id}/nutrition-prescriptions','/api/v1/admissions/{admission_id}/nutrition-intake','/api/v1/admissions/{admission_id}/nutrition-labs'}; assert expected<=set(p),expected-set(p); assert not any('audit_logs' in path for path in p); print('OpenAPI clinical contracts valid')"
    Assert-NativeSuccess "OpenAPI"

    Write-Host "[5/12] metadata tables, constraints, precision and indexes"
    python -c "from app.db.base import get_metadata; m=get_metadata(); expected={'nutritional_care_encounters','nutritional_assessments','nutritional_clinical_context_items','nutritional_anthropometric_measurements','nutritional_screenings','nutritional_screening_answers','nutritional_requirement_calculations','nutritional_diagnoses','nutritional_prescriptions','nutritional_prescription_meal_times','nutritional_monitoring_records','nutritional_intake_records','nutritional_lab_observations','nutritional_alerts'}; assert expected<=set(m.tables),expected-set(m.tables); e=m.tables['nutritional_care_encounters']; assert {'admission_id','encounter_datetime','status','version'}<=set(e.c.keys()); assert any(i.name=='ix_nutrition_encounter_admission_datetime_status' for i in e.indexes); assert str(m.tables['nutritional_anthropometric_measurements'].c.value.type).startswith('NUMERIC'); print('Phase 9 metadata valid')"
    Assert-NativeSuccess "Metadata"

    Write-Host "[6/12] isolated migration upgrade, downgrade and re-upgrade"
    Push-Location "apps/backend"
    python -m pytest tests/test_phase9_migration.py -q
